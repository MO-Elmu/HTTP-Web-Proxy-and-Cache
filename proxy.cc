...
void HTTPProxy::acceptAndProxyRequest() throw (HTTPProxyException) {
  ...
  try {
    if(isUsingProxy()) scheduler.scheduleRequest(connectionfd, clientIPAddress, proxyServer, proxyPortNumber);
    else scheduler.scheduleRequest(connectionfd, clientIPAddress);
  } catch (...) {
